...
const MIMI_PROVIDER_STARTUP_CATEGORY_TIMEOUT_MS = providerRuntimeNumber(
  "MIMI_PROVIDER_STARTUP_CATEGORY_TIMEOUT_MS",
  1200
);
const MIMI_PROVIDER_ACTIVE_REQUEST_BOOT_TIMEOUT_MS = providerRuntimeNumber(
  "MIMI_PROVIDER_ACTIVE_REQUEST_BOOT_TIMEOUT_MS",
  1800
);
...
import {
  ...
  verifyOtp
} from "./services/service-api.js?v=2026.06.05.2";
import {
...
import {
  ...
  qualityMessage
} from "./utils/document-image-quality.js?v=2026.06.05.2";

import {
  resolverDireccionActualServicio
} from "./services/service-geocoding.js?v=2026.06.05.2";
...
} from "./services/push.js?v=2026.06.05.2";
import {
...
function withProviderBootTimeout(promise, ms, label = "PROVIDER_BOOT_TIMEOUT") {
...
class MimiProviderApp {
  constructor() {
    ...
    this.providerBootstrapSession = null;
...
async init() {
  ...
  try {
    ...
    this.providerBootstrapSession = earlySession?.isAuthenticated ? earlySession : null;
  ...
  try {
    ...
    const cats = await withProviderBootTimeout(
      loadCategories(),
      MIMI_PROVIDER_STARTUP_CATEGORY_TIMEOUT_MS,
      "PROVIDER_STARTUP_CATEGORIES_TIMEOUT"
    );
...
async loadProviderBootResource(label, loader, fallback, warnings = [], timeoutMs = 0) {
  let failure = null;
  let failed = false;
  const metricLabel = providerBootResourceMetricLabel(label);
  markProviderBoot(`${metricLabel}.start`);
  try {
    const value = await withProviderBootTimeout(
...
consumeProviderBootstrapSession() {
  const session = this.providerBootstrapSession;
  this.providerBootstrapSession = null;
...
async loadInitialData() {
  ...
  try {
    ...
    try {
      session = this.consumeProviderBootstrapSession();
      if (!session) {
...
if (Array.isArray(categories) && categories.length) {
  ...
  actions.updateState({
    ...
    const hydratedWorkspace = {
      ...workspace,
      profile: workspace?.profile ?? session?.providerProfile ?? null
    };
    this.applyWorkspaceToState(hydratedWorkspace);
    ...
    if (bootWarnings.some((item) => item.label === "servicio activo")) {
      window.setTimeout(() => {
        this.resyncActiveService("boot_active_request_retry").catch((error) => {
          console.warn("[MIMI Provider] Reintento diferido de servicio activo no disponible:", error?.message || error);
        });
      }, 0);
    }
...
normalizeProviderOfferingPayload(offering = {}) {
  const normalized = {
    ...offering,
...
collectProviderBusinessPayload(form) {
  ...
  for (const existing of existingOfferings) {
    ...
    offerings.push(this.normalizeProviderOfferingPayload({
      id: existing.id,
