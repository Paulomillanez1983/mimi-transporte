...
function renderAuth(state) {
  ...
  if (authHint) {
    authHint.textContent = isAuthenticated
      ? "Ya podés ver ofertas, actualizar estados y operar tu servicio activo."
      : hasBackend
        ? "Ingresá con Google para ver tus ofertas y operar en tiempo real."
...
export function renderProviderStats(state) {
  ...
  document.querySelectorAll("[data-provider-status]").forEach((button) => {
    button.classList.toggle(
      "is-active",
      button.dataset.providerStatus === state.provider.status
    );
  });
}
  ...
  providerActiveService.innerHTML = activeService
    ...
    : `
      <div class="summary-card">
        ...
        <span class="muted">Aceptá una oferta para habilitar las acciones operativas.</span>
      </div>
    `;
