...
export function lifecycleLog(event: string, metadata: Record<string, unknown> = {}) {
  console.log(JSON.stringify({
    area: "mimi_services_lifecycle",
    event,
...
export function sanitizeLifecycleRequest<T extends Record<string, unknown> | null | undefined>(request: T) {
  if (!request) return request;

  const sanitized = { ...request };
  delete sanitized.service_pin_hash;
  delete sanitized.service_pin_ciphertext;
  delete sanitized.service_pin_attempts;
  delete sanitized.service_pin_locked_until;
...
export async function handleProviderTransition(req: Request, config: TransitionConfig) {
  ...
  try {
    ...
    if (request.status === config.targetStatus) {
      const sanitizedRequest = sanitizeLifecycleRequest(request);
      return json({ ok: true, already_processed: true, request: sanitizedRequest, service: sanitizedRequest, correlation_id: correlationId });
    }

    if (!config.allowedStatuses.includes(String(request.status))) {
      lifecycleLog("transition_rejected", {
        correlation_id: correlationId,
        function_name: config.functionName,
        request_id: requestId,
        provider_id: provider.id,
        current_status: request.status,
        target_status: config.targetStatus,
    ...
    lifecycleLog("transition_applied", {
      correlation_id: correlationId,
      function_name: config.functionName,
      request_id: requestId,
      provider_id: provider.id,
    ...
    const sanitizedRequest = sanitizeLifecycleRequest(updatedRequest);
    return json({ ok: true, request: sanitizedRequest, service: sanitizedRequest, correlation_id: correlationId });
  } catch (error) {
...
export async function handleProviderComplete(req: Request) {
  ...
  try {
    ...
    if (request.status !== "IN_PROGRESS") {
      lifecycleLog("transition_rejected", {
        correlation_id: correlationId,
        function_name: "svc-complete-service",
        request_id: requestId,
        provider_id: provider.id,
        current_status: request.status,
    ...
    const sanitizedRequest = sanitizeLifecycleRequest(updatedRequest);
    return json({ ok: true, result, request: sanitizedRequest, service: sanitizedRequest, correlation_id: correlationId });
  } catch (error) {
