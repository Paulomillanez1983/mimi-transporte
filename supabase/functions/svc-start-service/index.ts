  ...
  try {
    ...
    if (!assertUuid(requestId)) {
      lifecycleLog("transition_rejected", {
        ...
        function_name: "svc-start-service",
        reason: "request_id_invalid",
      });
      return json({ ok: false, error: "request_id_invalid", correlation_id: correlationId }, 400);
    }
    ...
    if (!["PROVIDER_ARRIVED", "PROVIDER_EN_ROUTE", "ACCEPTED", "SCHEDULED"].includes(String(request.status))) {
      ...
      return json({
        ...
        expected: ["PROVIDER_ARRIVED", "PROVIDER_EN_ROUTE", "ACCEPTED", "SCHEDULED"],
        correlation_id: correlationId,
      }, 409);
    }
    ...
    if (request.service_pin_expires_at && new Date(request.service_pin_expires_at).getTime() < Date.now()) {
      return json({ ok: false, error: "pin_expired", correlation_id: correlationId }, 409);
    }
    if (request.service_pin_locked_until && new Date(request.service_pin_locked_until).getTime() > Date.now()) {
      return json({ ok: false, error: "pin_temporarily_locked", correlation_id: correlationId }, 429);
    }
    ...
    if (submittedHash !== request.service_pin_hash) {
      ...
      lifecycleLog("pin_validation_failed", {
        correlation_id: correlationId,
        request_id: requestId,
        provider_id: provider.id,
        attempts,
      ...
      return json({ ok: false, error: lockedUntil ? "pin_temporarily_locked" : "pin_incorrect", attempts, correlation_id: correlationId }, lockedUntil ? 429 : 409);
    }
